...
<?php 
	...
	function verifica_filho($idPai, $idFilho){
		...
		// $idPai = str_replace(" ",'',$idPai);
		$idFilho = str_replace(" ",'',$idFilho);
		...
		foreach ($idPai as $key => $value) {
			...
			// echo ' |   ver filho ';
			// var_dump($idFilho[$key]);
			// echo '<br></br>';
			
			//corre array do indice pai
			if(strcmp($value, $idFilho[$key]) != 0){

				// nivel superior diferente nivel pai
				return false;
			}
		}
		// quer dizer que o nivel superior é igual
		return true;
	}
	...
	function get_children($id, $matrix){
		...
		foreach ($matrix as $key => $value) {
			if(verifica_filho($id, $value['id_tarefa'])){
				// pega os filhos dessa matrix indicada pelo id
	...
	function escreve_array($pais, $matrix, $nivel){
		...
		foreach ($pais as $key => $value) {
			...
			// echo '<br></br>';
			// // echo '<br>s niv </br>';
			// // var_dump($matrix[($nivel)]);
			// echo '<br></br>';
			// echo '<br> niv +1</br>';
			// var_dump($matrix[($nivel+1)]);
			// echo '<br></br>';


			// se houver esse nivel a mais na matriz
			if(isset($matrix[($nivel+1)])){
				// se a matrix com esse id tiver o filho
				// echo '  di taref <br></br>';
				// var_dump($value['id_tarefa']);
				// echo '<br></br>';
				// echo '<br> niv +1</br>';
				// var_dump($matrix[($nivel+1)]);
				// echo '<br></br>';
	        ...
	        foreach ($Reader as $registro => $linha ){  //Para percorrer as linhas dentro da planilha
	            ...
	            if ($registro > 4){
	                ...
	                for($i = 0; $i < count($linha); $i++){
	                    ...
	                    if ($i == 0) {  
	                        if (empty($linha[$i])) {
								...
								if($no['id'][1] == "." && $no['id'][0] == " " ){
									$no['id']= substr($no['id'], 2);
								}
								array_push($vara, [[$no['id'],$registro]]);
								// echo '<br></br>';
	                    ...
	                    elseif ($i == 1) {
	                        if (empty($linha[$i])) {
	                            ...
	                            $no['descricao'] = $linha[$i];
								//$no['descricao'] = utf8_decode($linha[$i]);
								array_push($varb, [[$no['descricao'],$registro]]);
	                    ...
	                    elseif ($i == 2) {
	                        if(empty($linha[$i])){
	                            ...
	                            array_push($vaai,abs($linha[$i]));
	                            
	                            if(floatval($linha[$i]) > 0){
							...
							array_push($varc, [[$no['qnt'],$registro]]);
	                    ...
	                    elseif ($i == 4) {
	                        if (empty($linha[$i])) {
								...
								array_push($var,[$no['valor_uni'],$registro]);
	                        }
	                    }
	                ...
	                if ($valida == 1) {
						...
						// var_dump($no['descricao']);
						// $no['descricao'] = addslashes($no['descricao']);
    ...
    if(isset($_POST['orcamento'])){
		...
		// echo 'comeco ';
		// echo $_POST['orcamento'];
		// var_dump($nos_material);
		// var_dump($nos_tarefa);
		...
		foreach($nos_material as $key => $value) {
			...
			if(count($value) < 6){
				...
				$value[1]= addslashes(strtoupper($value[1]));
				...
				if(!empty($value[4]) or !empty($value[3])){
					...
					$value[1] = addslashes(utf8_decode($value[1]));
