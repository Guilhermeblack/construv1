...
<body>
	<script type="text/javascript">
		...
		function aprovar_pagamento(){
			...
			$('input[name="string_consulta"]').val(window.location.href);

			document.nome.action = "const_avencer_parcela.php";
			document.nome.submit();
		}
	</script>
	...
	<div id="page-container" class="page-container fade page-sidebar-fixed page-header-fixed">
		...
		<div id="content" class="content">
			...
			<ol class="breadcrumb pull-right">
				...
				<?php if (in_array('41', $idrota)) { ?>
					<li><a href="#" onclick="estornar_pagamento()"><span class="label label-warning">ESTORNAR PAGAMENTO</span></a></li>
				<?php } ?>

				<?php if (in_array('7', $idrota)) { ?>
					<li><a href="#" onclick="receber()"><span class="label label-success">BAIXAR PARCELA</span></a></li>
				<?php } ?>
				<?php if (in_array('6', $idrota)) { ?>
					<li><a href="lancar_conta_pagar.php"><span class="label label-success">NOVO LANÇAMENTO</span></a></li>
				<?php } ?>

				<li><a href="#" onclick="aprovar_pagamento()"><span class="label label-warning">APROVAR PAGAMENTO</span></a></li>
				...
				<div class="col-md-12">
					<div class="panel panel-inverse">
						...
						<div class="panel-body">
							<form class="form-vertical form-bordered" method="GET" action="contas_apagar.php">
								...
								<div class="row">
									<div class="col-md-6">
										<div class="form-group">
											<label class="col-md-2 control-label">Centro de Receita</label>
											<select class="default-select2 form-control" name="centrocusto_id" >
												...
												if(mysqli_num_rows($query)){
													while ($assoc = mysqli_fetch_assoc($query)) {
														?>
									            		<option value="<?php echo $assoc['id'] ?>" tipo="etapa"> <?php echo $assoc['codigo']."/".$assoc['descricao'];?> </option>
														<?php
													}
												}
									            ...
									            while ($buscar_amigo = mysqli_fetch_assoc($executa_query)) {//--verifica se são amigos
									            	...
									            	?>
									            	<option value="<?php echo "$idcentro" ?>" tipo="centro_receita"> <?php echo "$ref "."/ "."$desc_centro" ?> </option>
									            <?php } ?>
									...
									<div class="col-md-6">
										<div class="form-group">
											<label class="col-md-2 control-label">Nº Nota Fiscal</label>
											<input type="text" class="form-control" name="numero_boleto"  />
										</div>
									</div>
								</div>
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											<label class="control-label">Tipo Periodo</label>
									...
									<div class="col-md-3">
										<div class="form-group">
											...
											<div class="input-group">
												<input type="radio" name="situacao" value="4" required="">Previsão
					...
					<div class="panel panel-inverse">
						...
						<div class="panel-body">
							...
							<form action="receber_parcelas_apagar.php" method="POST" id="nome" name="nome">
								...
								$numero_boleto            = $_GET["numero_boleto"];
								$centrocusto_id            = $_GET["centrocusto_id"];
								...
								if($situacao == 1){
									...
									$situacao_and = " AND situacao = 'Previsao'";
								}else{
									$situacao_and = '';
								}

								if($numero_boleto != ''){
									$situacao_and .= " AND numero_boleto =".$numero_boleto;

								}

								if($centrocusto_id != ''){
									$situacao_and .= " AND centrocusto_id =".$centrocusto_id;
								...
								<input type="hidden" name="string_consulta" value="">
								...
								<table id="data-table" class="table table-striped table-bordered">
									...
									<tbody>
										...
										include "conexao.php";
										$query = "SELECT numero_boleto, parcelas.idparcelas, parcelas.desc_parcela, parcelas.acre_parcela ,data_lancamento_sistema, lancamento_por, baixado_por, data_baixa,parcelas.tipo_venda, parcelas.venda_idvenda, parcelas.valor_recebido, parcelas.valor_parcelas, parcelas.data_recebimento,parcelas.situacao, parcelas.empreendimento_id_novo, parcelas.cliente_id_novo,parcelas.cod_baixa,parcelas.forma_pagamento, STR_TO_DATE(data_vencimento_parcela, '%d-%m-%Y') as venc  
										FROM parcelas 
										WHERE fluxo = 1  $situacao_and order by venc, idparcelas Asc";
			                ...
			                while ($buscar_amigo = mysqli_fetch_assoc($executa_query)) {//--verifica se são amigos
								...
								// $fornec = mysqli_query($db,"SELECT nome_cli FROM `cliente` where `id` = $cliente_idcliente");
								// $nome_fornec = mysqli_fetch_assoc($fornec)

								$desc_emp 					=$buscar_amigo['empreendimento_id_novo'];
								$fornec						=$buscar_amigo['cliente_id_novo'];
			                	...
			                	$oc_ci                      = $buscar_amigo["oc_ci"];
			                	$numero_ci                  = $buscar_amigo["numero_ci"];
			                	$numero_boleto              = $buscar_amigo["numero_boleto"];
			                	...
			                	if($situacao_parcela == 'Previsao'){
			                		$texto_situacao = '<span class="label label-warning">Previsão de Pagamento</span>';
			                	}else
								...
								include "conexao.php";
								$fornecedor = mysqli_query($db,"SELECT nome_cli from cliente where `idcliente` = $fornec");
								$fornec_nome= mysqli_fetch_assoc($fornecedor);

								$emp = mysqli_query($db, "SELECT descricao_empreendimento FROM empreendimento_cadastro WHERE idempreendimento_cadastro = $desc_emp");
								$emp_nome= mysqli_fetch_assoc($emp);
			                	...
			                	<tr class="odd gradeX">
			                		...
			                		<td><?php echo $fornec_nome['nome_cli']; ?></td>
			                		<td><?php echo $numero_boleto ?></td>



			                		<td><?php echo $emp_nome['descricao_empreendimento']; ?></td>
			                		...
			                		<td>
			                			<a href="anexar_parcelas.php?idparcelas=<?php echo $idparcelas ?>"><span class="label label-primary">Anexar Documento</span></a>
	...
	<div id="atencao" class="modal fade" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				...
				<div class="modal-body"  align="center">
					<h4 class="modal-title" align="center">Selecione Somentes Parcelas de Previsão!</h4>
				</div>
...
<script>
	$(document).ready(function() {
		...
		<?php 
			...
			if(isset($_GET['atencao'])){
				?>
					$('div#atencao').modal('show');			
				<?php
			}
