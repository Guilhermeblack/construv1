...
$proposta_compra             = $buscar_amigo["distrato"];
...
<h3 align='center'><u>DISTRATO DE COMPROMISSO DE COMPRA E VENDA DE LOTE</u></h3>
<h5 align='center'>O presente instrumento de distrato tem como objetivo rescindir um Contrato de Compromisso de Compra e Venda de Lote no Loteamento $descricao_empreendimento , em que as partes abaixo qualificadas firmaram entre si na data de: $data_venda </h5>
...
<h4 style='margin:10px !important'>2-QUALIFICAÇÃO DO(S) CONTRATANTES(S) / VENDEDORES(A)(ES)(AS):</h4>
<table border='0' style='width:100%' align='center' cellspacing='0' cellpadding='0' summary='Tabela de informações de contato'height='6'>
  <tr>
    <td height='6'>Nome: $nome_emp</td>
    <td height='6'><strong>REPRESENTADA POR:</strong></td>
  </tr>
  <tr>
    <td height='6'>CNPJ: $cpf_emp</td>
    <td height='6'>Nome: $nome_rep</td>
  </tr>
  <tr>
    <td height='6'>Inscricão Municipal: $insc_municipal_emp</td>
    <td height='6'>RG: $rg_rep</td>
  </tr>
  <tr>
    <td height='6'>Inscrição Estadual: $rg_emp</td>
    <td height='6'>CPF / CNPJ: $cpf_rep</td>
  </tr>
  <tr>
    <td height='6'>Endereço: $endereco_emp</td>
    <td height='6'>Nacionalidade: $nacionalidade_rep</td>
  </tr>
  <tr>
    <td height='6'>Bairro: $bairro_emp</td>
    <td height='6'>Profissão: $profissao_rep</td>
  </tr>
  <tr>
    <td height='6'>Cidade: $cidade_emp</td>
    <td height='6'>Estado Civil: $estadocivil_rep</td>
  </tr>

  <tr>
    <td height='6'>UF: $estado_emp</td>
    <td height='6'>Endereço $endereco_rep</td>  
  </tr>

  <tr>
    <td height='6'>Email: $email_emp</td>
    <td height='6'>Bairro: $bairro_rep</td> 
  </tr>
  <tr>
    <td height='6'></td>
    <td height='6'>Cidade: $cidade_rep</td>
  </tr>
  <tr>
    <td height='6'></td>
    <td height='6'>UF: $estado_rep</td>
  </tr>

</table><br>
<h4 style='margin:10px !important'>3-DESCRIÇÃO DO LOTE OBJETO DESTE CONTRATO:</h4>
...
<h4 style='margin:10px !important'><strong>4- VALOR E FORMA DE PAGAMENTO DA RENEGOCIAÇÃO:</strong></h4>
<p width='100%'>";
       

       include "../conexao.php";
       $query_primeira = "SELECT * FROM distrato where venda_id = $idvenda order by iddistrato desc limit 1";
       ...
       while ($buscar_primeira = mysqli_fetch_assoc($executa_primeira)) {
         ...
         $valor_amortizado = $buscar_primeira["valor_amortizado"];
         $valor_lote       = $buscar_primeira["valor_lote"];
         $valor_custo      = $buscar_primeira["valor_custo"];
         $valor_devolver   = $buscar_primeira["valor_devolver"];

         $valor_amortizado = 'R$ ' . number_format($valor_amortizado, 2, ',', '.'); 
         $valor_lote       = 'R$ ' . number_format($valor_lote, 2, ',', '.'); 
         $valor_custo      = 'R$ ' . number_format($valor_custo, 2, ',', '.'); 
         $valor_devolver   = 'R$ ' . number_format($valor_devolver, 2, ',', '.'); 

         $obs              = $dados_parcela1["obs"];
         $data_distrato    = $dados_parcela1["data_distrato"];
  ...
  $html .="<p>
              <div align='justify'>Valor Total do Contrato: $valor_lote</div>
          </p>";
  

    $html .="<p>
             <div align='justify'>Valor Total Pago: $valor_amortizado</div>
          </p>";

    $html .="<p>
              <div align='justify'>Valor Total Custo Distrato: $rene_valor_parcela</div>
          </p>";

    $html .="<p>
              <div align='justify'>Valor de Devolução: $valor_devolver</div>
          </p>";


    $html .="<p>
              <div align='justify'>Observação: $obs</div>
          </p>";
