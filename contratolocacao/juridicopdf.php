...
$horario_relatorio = date('d-m-Y H:i:s');
 ...
 // // echo  "Qual é a situacao = " . $situacao ; die();
...
if($cliente_id != "" && $teste != ""){
    ...
    $where =" AND cliente_id_novo =  $cliente_id  AND empreendimento_id_novo = $teste";


}


if($cliente_id != "" && $teste == 0){
   // echo "entrei aqui"; die();
    //caso tenha feito a busca por cliente entra aqui
    $where =" AND cliente_id_novo = ".$cliente_id;
}

if($teste != "" && $cliente_id == ""){
    //caso tenha feito a busca por empreendimento
...
if($cliente_id != "" && $teste != ""){
 ...
 $dados_cliente  =  dados_cliente($cliente_id); 
  $nomecliente = $dados_cliente['nome_cli'];

      $html = "<table style='width:100%' border='0' align='center'>

    <tr>
      <td style='width:20%'><img src='../img/$empreendimento_id/$img' height='110' /></td>
      <td style='width:60%'><center><h4>RELATÓRIO JURÍDICO</h4>
...
$hoje = date('Y-m-d');
  include "../conexao.php";
            $query = "SELECT parcelas.idparcelas, parcelas.tipo_venda, parcelas.venda_idvenda, parcelas.valor_recebido, parcelas.valor_parcelas, parcelas.data_recebimento,parcelas.situacao,parcelas.cod_baixa,parcelas.cliente_id_novo, parcelas.forma_pagamento, STR_TO_DATE(data_vencimento_parcela, '%d-%m-%Y') as venc 
                      FROM parcelas  
                      WHERE fluxo = 0 AND situacao = 'Em Aberto' AND tipo_venda = 2 AND STR_TO_DATE(data_vencimento_parcela, '%d-%m-%Y') <  DATE_SUB(CURDATE(), INTERVAL 90 DAY) $where order by cliente_id_novo, venda_idvenda, venc Asc";
            //echo $query; die();
