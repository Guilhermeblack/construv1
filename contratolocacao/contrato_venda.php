...
if($quem_vendeu == 0){
  ...
  $nome_exibe  = $dados_vendeu["nome_cli"];
...
<h4 style='margin:10px !important'>1-QUALIFICAÇÃO DO(S) COMPROMISSARIO(S) / COMPRADOR (A)(ES)(AS):</h4>
...
<table border='0' cellspacing='0' cellpadding='0' summary='Tabela de metas de longo prazo' width='100%'>
  <tr>
    <td>3.1- Valor total do contrato: $valor_total_do_contrato</td>
  </tr>";

                $query_entrada = "SELECT * FROM parcelas where fluxo = 0 AND venda_idvenda =  $idvenda and tipo_venda = 2 AND (descricao = 'Parcelamento Entrada' or descricao = 'Primeira da Entrada')order by numero_sequencia Asc";
            ...
            $total_entrada_lote = $total_entrada_lote + $valor_buscar_entrada;
                    ...
                    $total_entrada_lote_exibir = 'R$ ' . number_format($total_entrada_lote, 2, ',', '.'); 

  $html .="<tr>
    <td><div align='justify'>3.2- Entrada: $total_entrada_lote_exibir, Dividida em $cont_entrada parcelas mensais <br>";
include "../conexao.php";
                $query_entrada = "SELECT * FROM parcelas where fluxo = 0 AND venda_idvenda =  $idvenda and tipo_venda = 2 AND (descricao = 'Parcelamento Entrada' or descricao = 'Primeira da Entrada') order by numero_sequencia Asc";
...
if($plano_pagamento != '' AND $plano_pagamento != 0){
 $html.="<tr>
    <td>3.3- Saldo Restante em: $plano_pagamento prestações mensais    de $valor_parcela_financiamento    vencendo a primeira em $data_vencimento_parcela2,    e as demais vencendo no mesmo dia dos meses subsequentes.</td>
  </tr>";
}
$html.="</table><BR><BR>";
