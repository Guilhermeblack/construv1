...
function dados_contrato($idparcelas){
  ...
  while ($busca_dados = mysqli_fetch_assoc($executa_query)) {
         ...
         $endereco          = $busca_dados['endereco'];
         $numero            = $busca_dados['numero'];
         $cep               = $busca_dados['cep'];
...
$dompdf = new DOMPDF();
    ...
    <tr>
      ...
      <td style='width:60%'><center><h4>RELATÓRIO DE INADIMPLENTES LOCAÇÃO</h4>
...
<table style='width:100%' height='127' border='0'>
    ...
    <tr bgcolor='#b8b8b8'>
      <td colspan='2' style='text-align: center'><strong>Endereço</strong></td>
...
$hoje = date('Y-m-d');
  include "../conexao.php";
            $query = "SELECT parcelas.idparcelas, parcelas.tipo_venda, parcelas.venda_idvenda, parcelas.valor_recebido, parcelas.valor_parcelas, parcelas.data_recebimento,parcelas.situacao,parcelas.cod_baixa,parcelas.cliente_id_novo, parcelas.forma_pagamento, STR_TO_DATE(data_vencimento_parcela, '%d-%m-%Y') as venc 
                      FROM parcelas  
                      WHERE fluxo = 0 AND situacao = 'Em Aberto' AND tipo_venda = 1  $where order by cliente_id_novo, venda_idvenda, venc Asc";
            ...
            while ($buscar_amigo = mysqli_fetch_assoc($executa_query)) {//--verifica se são amigos
              ...
              $endereco = $dados_contrato["endereco"];
              $numero   = $dados_contrato["numero"];
              $cep      = $dados_contrato["cep"];
...
if($cont_linha > 0){
 ...
 $html .="<tr>
  <td colspan='5' align='right'><strong>Subtotal do Imovel:</strong></td>
...
if($venda_ant != $venda_idvenda AND $cont_linha > 0){
  ...
  $html .="<tr>
  <td colspan='5' align='right'><strong>Subtotal do Imóvel:</strong></td>
  <td style='text-align: center'><strong>  $cont_lote_e </strong></td>
    <td style='text-align: center'><strong> $cont_multa_e</strong></td>
  <td style='text-align: center'><strong> $cont_juros_e</strong></td>
  <td style='text-align: center'><strong> $cont_multa_juros_e</strong></td>
  ...
  $cont_multa_juros = 0;
   ...
   $html .="<tr>
      <td colspan='2' style='text-align: left'>$endereco , $numero</td>
