...
$valor_para_parcelamento      = $buscar_amigo["valor_para_parcelamento"];   // VALOR DO LOTE A SER FINANCI   
...
$valor_desconto = 'R$ ' . number_format($valor_desconto, 2, ',', '.'); 
$valor_para_parcelamento = 'R$ ' . number_format($valor_para_parcelamento, 2, ',', '.'); 
...
function ids_socio($codigo_cessao){

  include "../conexao.php";
  $query_cessao = mysqli_query($db, "SELECT * FROM proprietarios_lote
                      WHERE cod_cessao = $codigo_cessao");
     $cont = 0;   
   while ($buscar_amigo = mysqli_fetch_assoc($query_cessao)) {
               
      $cliente_id[$cont]           = $buscar_amigo["cliente_id"];
...
return $dados;
 ...
 include "../conexao.php";
   $query_contrato = mysqli_query($db, "SELECT conteudo FROM modelo_contrato where idmodelo_contrato = $modelo_id");
...
foreach ($total_socio as $key) {
  ...
  $a = array('@nome'.$num.'_cli', '@rg'.$num.'_cli', '@cpf'.$num.'_cli', '@nacionalidade'.$num.'_cli', '@profissao'.$num.'_cli', '@nascimento'.$num.'_cli', '@estadocivil'.$num.'_cli', '@renda_total','@cep'.$num.'_cli', '@endereco'.$num.'_cli', '@numero'.$num.'_cli', '@bairro'.$num.'_cli', '@cidade'.$num.'_cli', '@estado'.$num.'_cli', '@telefone1'.$num.'_cli', '@telefone2'.$num.'_cli', '@nome'.$num.'_con', '@rg'.$num.'_con', '@cpf'.$num.'_con', '@nacionalidade'.$num.'_con', '@profissao'.$num.'_con', '@nascimento'.$num.'_con', '@telefone1'.$num.'_con', '@telefone2'.$num.'_con','@descricao_empreendimento', '@matricula_empreendimento', '@quadra', '@lote', '@m2', '@cadastro_prefeitura', '@matricula', '@confrontacao', '@valor_sinal', '@valor_entrada', '@qtd_parcelas_entrada', '@valor_parcela_entrada', '@vencimento_primeira','@qtd_parcelas_financiamento', '@valor_parcela_financiamento', '@data_vencimento_primeira_financiamento', '@vendedora', '@dia_contrato', '@mes_contrato', '@ano_contrato','@dia_vencimento_primeira_financiamento', '@email_cli', '@valor_lote', '@valor_para_parcelamento'); 


    
  $dados_proponente = dados_proponente($key);

$nome_cli           = $dados_proponente["nome_cli"];
$cpf_cli            = $dados_proponente["cpf_cli"];
$rg_cli             = $dados_proponente["rg_cli"];
$estadocivil_cli    = $dados_proponente["estadocivil_cli"];
$nacionalidade_cli  = $dados_proponente["nacionalidade_cli"];
$profissao_cli      = $dados_proponente["profissao_cli"];
$nascimento_cli     = $dados_proponente["nascimento_cli"];
$email_cli          = $dados_proponente["email_cli"];
$cidade_cli         = $dados_proponente["cidade_cli"];
$logradouro_cli     = $dados_proponente["logradouro_cli"];
$endereco_cli       = $dados_proponente["endereco_cli"];
$numero_cli         = $dados_proponente["numero_cli"];
$complemento_cli    = $dados_proponente["complemento_cli"];
$bairro_cli         = $dados_proponente["bairro_cli"];
$complemento_cli    = $dados_proponente["complemento_cli"];
$telefone1_cli      = $dados_proponente["telefone1_cli"];
$telefone2_cli      = $dados_proponente["telefone2_cli"];
$cep_cli            = $dados_proponente["cep_cli"];
$estado_cli         = $dados_proponente["estado_cli"];


 $tem_conjuge = tem_conjuge($key);
 $conjuge_id = $tem_conjuge["conjuge_idconjuge"];

  $dados_conjuge = dados_proponente($conjuge_id);

$nome_con           = $dados_conjuge["nome_cli"];
$cpf_con            = $dados_conjuge["cpf_cli"];
$rg_con             = $dados_conjuge["rg_cli"];
$estadocivil_con    = $dados_conjuge["estadocivil_cli"];
...
$email_con          = $dados_conjuge["email_cli"];
$telefone1_con      = $dados_conjuge["telefone1_cli"];
$telefone2_con      = $dados_conjuge["telefone2_cli"];




  $b = array($nome_cli, $rg_cli, $cpf_cli, $nacionalidade_cli, $profissao_cli, $nascimento_cli, $estadocivil_cli, $renda_total,$cep_cli, $endereco_cli, $numero_cli, $bairro_cli, $cidade_cli, $estado_cli, $telefone1_cli, $telefone2_cli, $nome_con, $rg_con, $cpf_con, $nacionalidade_con, $profissao_con, $nascimento_con, $telefone1_con, $telefone2_con,$descricao_empreendimento, $matricula_empreendimento, $quadra, $lote, $m2, $cadastro_prefeitura, $matricula, $confrontacao, $valor_sinal, $valor_entrada, $qtd_parcelas_entrada, $valor_parcela_entrada, $vencimento_primeira,$qtd_parcelas_financiamento, $valor_parcela_financiamento, $data_vencimento_primeira_financiamento, $vendedora, $dia_contrato, $mes_contrato, $ano_contrato, $dia_vencimento_primeira_financiamento, $email_cli, $valor_desconto, $valor_para_parcelamento); 
...
$ids_socio   = ids_socio($codigo_cessao);


 $trocar = trocar_conteudo($conteudo, $descricao_empreendimento, $matricula_empreendimento, $quadra, $lote, $m2, $cadastro_prefeitura, $matricula, $confrontacao, $valor_sinal, $valor_entrada, $qtd_parcelas_entrada, $valor_parcela_entrada, $vencimento_primeira,$qtd_parcelas_financiamento, $valor_parcela_financiamento, $inter_data, $vendedora, $dia_contrato, $nome_mes, $ano_contrato, $dia_vencimento_primeira_financiamento, $email_cli, $ids_socio, $valor_desconto, $valor_para_parcelamento);
...
$html ="<p style='text-align: center'>$trocar</p>
";
