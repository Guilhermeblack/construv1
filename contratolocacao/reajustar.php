...
// echo "</pre>"; die();
           ...
           $mes_inicial = date('m' , strtotime($inicio));
           $mes_final = date('m' , strtotime($fim));
...
function verifica_reajuste($idvenda, $data_venda, $igpm){
           ...
           $data_inicio = date('Y-m-d',strtotime("-334 day", strtotime($inicio)));
           $data_fim = date('Y-m-d',strtotime("-334 day", strtotime($fim)));
             // echo $data_inicio . "<br>";
             // echo $data_fim . "<br>";
...
// $where =  " venda.idvenda = $idvenda AND (STR_TO_DATE(igpm, '%d-%m-%Y') BETWEEN  '".$data_inicio."' and '".$data_fim."' OR STR_TO_DATE(venda.data_venda, '%d-%m-%Y') = STR_TO_DATE(igpm, '%d-%m-%Y') OR STR_TO_DATE(venda.igpm, '%d-%m-%Y') = NULL)";
    ...
    <tr>
      ...
      <td style='width:80%'><center><h4>RELATÓRIO CONTRATOS À REAJUSTAR <br><h4>$por_cadastro</h4>
       <h4> <strong>Empreendimento.: </strong>&nbsp;&nbsp;  $empreendimento</h4>
...
$mes_do_ano = array('Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro');

$teste = $n_meses+$comeco-1;

for ($i=$comeco; $i <= $teste; $i++) { 
  //echo $i. "<br>";
           ...
           foreach ($dados as $dado ) {
                    if($dado['mes_igpm'] == $i){
                    //echo "todos os contratos de janeiro" . $dado['idvenda'] . "<br>";
