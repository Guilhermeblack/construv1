...
$contacorrente_id = $_POST["contacorrente_id"];
    ...
    function busca_tipo($idparcelas){

    include "conexao.php";
    $busca_conta = mysqli_query($db, "SELECT tipo_venda, venda_idvenda
                                      FROM parcelas
    ...
    while ($conta = mysqli_fetch_assoc($busca_conta)) {
            ...
            $tipo_venda     = $conta["tipo_venda"];
            $venda_idvenda  = $conta["venda_idvenda"];
...
function conta_empreendimento($idvenda){
    ...
    include "conexao.php";
    $busca_conta = mysqli_query($db, "SELECT contacorrente_id, quadra, lote, descricao_empreendimento
                                      FROM venda
    ...
    while ($conta = mysqli_fetch_assoc($busca_conta)) {
            ...
            $quadra                     = $conta["quadra"];
            $lote                       = $conta["lote"];
            $descricao_empreendimento   = $conta["descricao_empreendimento"];

            $dados["contacorrente"]     = $contacorrente;
            $dados["quadra"]            = $quadra;
            $dados["lote"]              = $lote;
...
function todas_parcelas($idvenda, $idtipo){
include "conexao.php";
    $query_amigo = "SELECT numero_sequencia FROM parcelas
                    where venda_idvenda = $idvenda AND tipo_venda = $idtipo AND fluxo = 0 order by idparcelas desc limit 1";
...
$wes_hoje = date('dmy');
    ...
    foreach($antecipar as $id){
                      ...
                      $busca_tipo = busca_tipo($id);

            $idvenda    = $busca_tipo["venda_idvenda"];
            $tipo_venda = $busca_tipo["tipo_venda"];

             $todas_parcelas = todas_parcelas($idvenda, $tipo_venda);
...
if($tipo_venda == 1)
{
 $multa_vend = '10%';
             ...
             $dados = retorna_dados_cliente($id, $idvenda, $tipo_venda);
            ...
            if($tipo_venda == 2){ 
                $descri =  $dados['descricao_empreendimento'];
             } 

            if($tipo_venda == 1){ 
                $descri =  $dados['endereco'].", ".$dados['numero']." Cep: ".$dados['cep'];
                 } 
                                    ...
                                    <tr>
                                        <td class='wrapper'>
                                            ...
                                            <table class='six columns'>
                                                <tr>
                                                    <td>
                                                      <img src='http://hrsantos.com.br/carteira/fotos/hr.png' />
                                                    </td>
                                ...
                                <tr>
                                    <td class='wrapper'>
                                        <table class='twelve columns'>
                                            <tr>
                                                <td class='last'>
                                                    <h4>Olá $nome_cli, </h4>   <p>seu boleto referente a ";


                                                    $email .="$descri já está disponivel";

                                                    $email .="<br><br><a href='http://hrsantos.com.br/carteira/boletos/boleto_santander_banespa-mes-unico.php?antecipar[]=$id&contacorrente_id=$contacorrente_id'>Imprimir</a>
                                                    </p>
                                                </td>
                                            </tr>
                                        </table>
...
$nome_env = 'HR SANTOS';
$email_env = 'contato@hrsantos.com.br';


// emails para quem será enviado o formulário
  $emailenviar = $email_cli;
  $destino = $emailenviar;
  $assunto = "HR SANTOS | Empreendimentos Imobiliarios";

  // É necessário indicar que o formato do e-mail é html
  $headers  = 'MIME-Version: 1.0' . "\r\n";
      $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
      $headers .= 'From: '.$nome_env.' <'.$email_env.'>';
  //$headers .= "Bcc: $EmailPadrao\r\n";
  
  $enviaremail = mail($destino, $assunto, $email, $headers);

}
  if($enviaremail){
  $mgm = "E-MAIL ENVIADO COM SUCESSO! <br> ";
  echo " <meta http-equiv='refresh' content='1;URL=contas_areceber.php?envio=1'>";
  }else{
